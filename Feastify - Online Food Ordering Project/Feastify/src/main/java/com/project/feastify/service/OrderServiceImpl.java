...
public class OrderServiceImpl implements OrderService {
    ...
    // 🔒 TEMP HARDCODE FOR DEBUGGING (replace with real keys when deploying)
    private static final String RAZORPAY_KEY = "rzp_test_yAkZljdoR5xkpk";
    private static final String RAZORPAY_SECRET = "ts7RrrfaXDTRlNjYlaSB5PhJ";

    @Override
    public OrderResponse createOrderWithPayment(OrderRequest request) throws RazorpayException {
        System.out.println("🔐 Using Razorpay Key: " + RAZORPAY_KEY);
        System.out.println("🔐 Using Razorpay Secret: " + RAZORPAY_SECRET);
    ...
    private OrderResponse convertToResponse(OrderEntity newOrder) {
        return OrderResponse.builder()
                .id(newOrder.getId())
                .userId(newOrder.getUser() != null ? newOrder.getUser().getId() : null)
                .amount(newOrder.getAmount())
                .userAddress(newOrder.getUserAddress())
                .email(newOrder.getEmail())
                .phoneNumber(newOrder.getPhoneNumber())
    ...
    public void verifyPayment(Map<String, String> paymentData, String status) {
        ...
        if ("paid".equalsIgnoreCase(status) && existingOrder.getUser() != null) {
            cartRespository.deleteByUserId(existingOrder.getUser().getId());
        }
    }
